...
- LAN exchange has no cloud account, cloud cursor, remote backend, synchronized master password, or shared master password. The sender creates a short-lived local HTTP listener and QR payload; the payload carries an application-layer encrypted package, one-use transfer token, transfer key, and one-time package password.
...
- The sender's master password is used only for local re-authentication before export. It is not copied into the QR payload, HTTP transport, backup JSON, or receiver UI. The transfer package is wrapped by a generated one-time LAN package password, and accepted records and attachments are re-encrypted under the receiver's current vault key before local persistence.
...
- LAN import requires the one-time package password from the QR payload before imported ciphertext is accepted through the existing backup manifest verification path.
